...
public class Utils {
    ...
    public static Image[][] resample(Image[][] imagesNC, boolean[] isBinaryC, int[][] shapeN) {
        return IntStream.range(0, imagesNC.length)
                .mapToObj(idx ->  IntStream.range(0, imagesNC[idx].length)
                        .mapToObj(c -> Utils.resampleImage(imagesNC[idx][c], isBinaryC[c], shapeN.length==1 ? shapeN[0] : shapeN[idx]))
                        .toArray(Image[]::new))
                .toArray(Image[][]::new);
    }
    public static Image[] resample(Image[] imagesN, boolean isBinary, int[][] shapeN) {
        ...
        return IntStream.range(0, imagesN.length)
                .mapToObj(idx -> Utils.resampleImage(imagesN[idx], isBinary, shapeN.length==1 ? shapeN[0] : shapeN[idx]))
                .toArray(Image[]::new);
    }
