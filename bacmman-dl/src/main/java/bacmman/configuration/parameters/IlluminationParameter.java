...
public class IlluminationParameter extends GroupParameterAbstract<IlluminationParameter> implements PythonConfiguration, Hint {
    ...
    public IlluminationParameter(String name) {
        super(name);
        this.children = Arrays.asList(gaussianBlur, noiseIntensity, gaussianNoise, poissonNoise, speckleNoise, histoElasticDeformIntensity, histoElasticDeformNPoints, illuVariationIntensity, illuVariationNPoints, illumVariation2D);
        initChildList();
    }
    @Override
    ...
    public IlluminationParameter duplicate() {
        IlluminationParameter res = new IlluminationParameter(name);
