...
public class DistNet2D implements TrackerSegmenter, TestableProcessingPlugin, Hint, DLMetadataConfigurable {
        ...
        if (stores!=null) { // set log function
            ...
            Runnable logDistances = () -> minDist.forEach((o, pl) -> {
                ...
                for (Pair<TrackingObject, Double> p : pl) {
                    ...
                    String targetString = target.stream().map(so -> so.getRegion().getLabel()+"").collect( Collectors.joining( "," ) );
                    ...
                    for (SegmentedObject s : source) {
                        ...
                        s.getMeasurements().setStringValue("TrackingDist_"+count++, logString);
                    }
                }
            });
    ...
    protected void solveConflictingLinks(TrackMateInterface<TrackingObject> tmi, int frame, boolean linkWithPrev, boolean removeClusters) {
        ...
        while (it.hasNext()) {
            ...
            if (o.originalObjects !=null) { // this is a cluster
