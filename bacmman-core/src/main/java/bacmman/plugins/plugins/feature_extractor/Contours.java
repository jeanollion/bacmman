...
public class Contours implements FeatureExtractorConfigurable, FeatureExtractor.FeatureExtractorOneEntryPerTrack {
    ...
    public void configure(Stream<SegmentedObject> parentTrack, int objectClassIdx, Predicate<SegmentedObject> includeObject) {
        ...
        Map<SegmentedObject, ImageShort> coordinates = parentTrack.filter(includeObjectF).collect(Collectors.toMap(Function.identity(), p -> p.getRegion().getRoi().getFlattenExternalContoutCoordinates() )); //.setLocDelta(p.getBounds().xMin(), p.getBounds().yMin())
