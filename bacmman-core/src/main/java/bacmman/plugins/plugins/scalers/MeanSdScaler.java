...
public class MeanSdScaler implements HistogramScaler, Hint {
    ...
    public Image scale(Image image) {
        if (isConfigured()) return ImageOperations.affineOpAddMul(image, transformInputImage? TypeConverter.toFloatingPoint(image, false, false):null, 1 / sd, -mean);
    ...
    public Image reverseScale(Image image) {
        if (isConfigured()) return ImageOperations.affineOpMulAdd(image, transformInputImage?TypeConverter.toFloatingPoint(image, false, false):null, sd, mean);
