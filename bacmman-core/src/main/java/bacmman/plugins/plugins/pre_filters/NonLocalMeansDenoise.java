...
public class NonLocalMeansDenoise implements PreFilter, Filter, Hint {
    ...
    ConditionalParameter autoEstimateCond = new ConditionalParameter(autoEstimateSigma).setActionParameters("true", smoothingFactor).setActionParameters("false", sigma, smoothingFactor).setHint("This parameter will appear as invalid if not installed").addValidationFunction(p -> {
        try {
