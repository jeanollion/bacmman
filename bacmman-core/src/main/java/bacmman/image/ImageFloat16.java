...
public class ImageFloat16 extends ImageFloatingPoint<ImageFloat16> implements PrimitiveType.ShortType {
    ...
    @Override
    public ImageFloat16 getZPlane(int z) {
        if (z >=sizeZ) throw new IllegalArgumentException("Z-plane cannot be superior to sizeZ");
        else {
            ImageFloat16 res = new ImageFloat16(name+"_z"+String.format("%05d", z), sizeX, pixels[z]);
    ...
    @Override public DoubleStream streamPlane(int z) {
        return IntStream.range(0, sizeXY).mapToDouble(i->toHalfFloat(pixels[z][i]));
    }
