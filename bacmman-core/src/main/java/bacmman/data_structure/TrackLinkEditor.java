...
public class TrackLinkEditor {
    ...
    public void setTrackLinks(SegmentedObject prev, SegmentedObject next, boolean setPrev, boolean setNext, boolean propagateTrackHead) {
        if (editableObjectClassIdx>=0 && ( editableObjectClassIdx!=prev.getStructureIdx()||editableObjectClassIdx!=next.getStructureIdx())) throw new IllegalArgumentException("This object is not editable");
        prev.setTrackLinks(next, setPrev, setNext, propagateTrackHead, modifiedObjects);
    }
