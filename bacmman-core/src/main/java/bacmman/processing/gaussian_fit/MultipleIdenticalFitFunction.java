...
public class MultipleIdenticalFitFunction implements FitFunction {
    ...
    final boolean addConstant, fitConstant;
     ...
     */
    public MultipleIdenticalFitFunction(int nParams, int nFunctions, FitFunction function, boolean addConstant, boolean fitConstant) {
        this.function = function;
        this.nParams = nParams;
        this.Nfunctions = nFunctions;
        this.addConstant=addConstant;
        this.fitConstant = fitConstant;
        if (!addConstant && fitConstant) throw new IllegalArgumentException("If fit constant, addConstant should be true");
        if (addConstant) {
    ...
    public double grad(double[] pos, double[] params, int i) {
        ...
        if (funIdx==Nfunctions) return fitConstant ? 1 : 0;
    ...
    public double hessian(double[] pos, double[] params, int i, int i1) {
        ...
        if (funIdx==Nfunctions) return 0; // constant
        copyToBucket(params, funIdx);
