...
public class TransformationPluginParameter<T extends Transformation> extends PluginParameter<T> {
    ...
    public TransformationPluginParameter<T> setPlugin(T pluginInstance, boolean duplicateParameters) {
        if (pluginInstance instanceof Transformation.ConfigurableTransformation && !(pluginInstance instanceof Transformation.Filter)) initInputChannel();
    ...
    public int getInputChannel() {
        ...
        else if (Transformation.Filter.class.isAssignableFrom(getSelectedPluginClass())) return outputChannel.getSelectedIndex();
        return -1;
    }
