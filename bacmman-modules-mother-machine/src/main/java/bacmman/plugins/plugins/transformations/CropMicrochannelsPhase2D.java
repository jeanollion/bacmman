...
public class CropMicrochannelsPhase2D extends CropMicroChannels implements Hint, HintSimple, TestableOperation {
    ...
    NumberParameter yOpenedEndMargin = new BoundedNumberParameter("Lower end Y-margin", 0, 60).setEmphasized(true).setHint("The y-coordinate of the microchannel open end will be translated of this value towards the top of the image. Allows removing the bright line from the cropped image. A positive value will yield in smaller images and a negative value in larger images");
    NumberParameter yEndMarginUp = new BoundedNumberParameter("Upper end Y-margin", 0, 0).setEmphasized(true).setHint("The y-coordinate of the closed-end will be translated of this value towards the bottom of the image. A positive (respectively negative) value will yield in smaller (respectively larger) images");
