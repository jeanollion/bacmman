...
public class BacteriaFluoHessian extends BacteriaHessian<BacteriaFluoHessian> implements Hint, DevPlugin {
    ...
    EnumChoiceParameter<BacteriaFluo.THRESHOLD_COMPUTATION> foreThresholdMethod=  new EnumChoiceParameter<>("Foreground Threshold", BacteriaFluo.THRESHOLD_COMPUTATION.values(), BacteriaFluo.THRESHOLD_COMPUTATION.ROOT_TRACK);
    ConditionalParameter<BacteriaFluo.THRESHOLD_COMPUTATION> foreThldCond = new ConditionalParameter<>(foreThresholdMethod).setActionParameters(BacteriaFluo.THRESHOLD_COMPUTATION.CURRENT_FRAME, foreThresholderFrame).setActionParameters(BacteriaFluo.THRESHOLD_COMPUTATION.ROOT_TRACK, foreThresholder).setActionParameters(BacteriaFluo.THRESHOLD_COMPUTATION.PARENT_TRACK, foreThresholder).setHint("Threshold for foreground region selection after watershed partitioning on edge map. All regions with median value over this value are considered foreground. <br />If <em>CURRENT_FRAME</em> is selected, threshold will be computed at each frame. If <em>PARENT_BRANCH</em> is selected, threshold will be computed on the whole parent track. If <em>ROOT_TRACK</em> is selected, threshold will be computed on the whole root track, on raw images (no prefilters).<br />Configuration Hint: value is displayed on right click menu: <em>display thresholds</em> command. Tune the value using intermediate image <em>Foreground detection: Region values after partitioning</em>, only foreground regions should be over this value");
