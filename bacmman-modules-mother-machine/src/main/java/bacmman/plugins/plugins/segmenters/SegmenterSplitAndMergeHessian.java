...
public abstract class SegmenterSplitAndMergeHessian implements SegmenterSplitAndMerge, ObjectSplitter, TestableProcessingPlugin {
    static String voidThSimpleHint = "Parameter used for void microchannels detection during track pre-filter step: <br />Lower this value if no cells are found in some microchannel containing cells. On the contrary increase the value if false positive objects are segmented in void microchannels";
    NumberParameter vcThldForVoidMC = new BoundedNumberParameter("Variation coefficient threshold", 3, 0.085, 0, null).setEmphasized(true).setSimpleHint(voidThSimpleHint).setHint(voidThSimpleHint + "<br />Computation details: To assess if whole microchannel track is void: we first compute the coefficient of variation (CV) of the histogram of raw images of the whole microchannel track, using only pixels in the central line of each microchannel (1/3 of the width). If CV is inferior to the value of this parameter, the whole track is considered to be void. <br />If the track is not void according to the previous criterion, another criterion is applied to detect frames where a microchannel is void: an Otsu threshold is computed on the prefiltered images of the whole microchannel track. A microchannel is considered as void at a given frame if the CV of raw signal at this frame is inferior to this threshold and its mean value of prefiltered signal is superior to the Otsu threshold");
    ...
    NumberParameter splitThreshold = new BoundedNumberParameter("Split Threshold", 4, 0.3, 0, null).setEmphasized(true)
            .setHint("Threshold for splitting touching bacteria (step 3). <br />A criterion is computed for each adjacent region pair, and they are merged if this criterion is inferior to this threshold. <br />A lower value will lead to more separation of bacteria.<br /><br /><em>Configuration Hint</em>: Tune the value using intermediate image <em>Interface Values before merge by Hessian</em>, interface with a value over this threshold will not be merged<br /><br /><b>This parameter should be carefully calibrated for each experimental setup</b><br /><br /><em>Algorithmic Details: </em>Let's call <em>interface</em> the contact area between two regions, <em>mean(H)@Inter</em> the mean hessian value at the interface and <em>mean(PF)@Inter</em> the mean value of the pre-filtered image at the interface, and <em>BCK</em> an estimation of the background value. <be />Criterion is:  mean(H)@Inter / ( mean(PF)@Inter - BCK )<br />")
            .setSimpleHint("Threshold for splitting touching bacteria. <br />A criterion is computed for each adjacent region pair, and they are merged if this criterion is inferior to this threshold. <br />A lower value will lead to more separation of bacteria.<br /><br /><em>Configuration Hint</em>: Tune the value using intermediate image <em>Interface Values before merge by Hessian</em>, interface with a value over this threshold will not be merged<br /><br /><br /><b>This parameter should be carefully calibrated for each experimental setup</b><br />");
    PreFilterSequence edgeMap = new PreFilterSequence("Edge Map").add(new ImageFeature().setFeature(ImageFeature.Feature.GAUSS).setScale(1.5), new StandardDeviation(2).setMedianRadius(0)).setHint("Operations defining edges used in the first watershed procedure that is used to define foreground");
