...
public abstract class SegmenterSplitAndMergeHessian implements SegmenterSplitAndMerge, ObjectSplitter, TestableProcessingPlugin {
    ...
    NumberParameter hessianScale = new BoundedNumberParameter("Hessian scale", 1, 4, 1, 6).setEmphasized(true).setHint("In pixels. Lower value -> finner split, more sensitive to noise. Influences the value of split threshold parameter. <br />Configuration Hint: tune this value using the intermediate image <em>Hessian</em>");
    NumberParameter splitThreshold = new BoundedNumberParameter("Split Threshold", 4, 0.3, 0, null).setEmphasized(true).setHint("Threshold for Split/Merge step. <br />Two regions A and B are merged if mean(hessian) @ interface(A-B) / mean(intensity) @ interface(A-B) is inferior to this parameter. <br />Lower value splits more.  <br />Configuration Hint: Tune the value using intermediate image <em>Interface Values before merge by Hessian</em>, interface with a value over this threshold will not be merged<br />This parameter should be carefully calibrated for each experimental setup");
