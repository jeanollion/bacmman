...
public class SpotSegmenter implements Segmenter, TrackConfigurable<SpotSegmenter>, ManualSegmenter, ObjectSplitter, TestableProcessingPlugin, Hint {
    ...
    NumberParameter thresholdHigh = new NumberParameter<>("Seed Laplacian Threshold", 2, 2.25).setEmphasized(true).setHint("Laplacian Threshold for seed selection.<br />Higher values tend increase false negative and decrease false positives.<br /> Configuration hint: corresponds to values in <em>Laplacian</em> image");
    ...
    NumberParameter intensityThreshold = new NumberParameter<>("Seed Threshold", 2, 1.6).setEmphasized(true).setHint("Intensity threshold for seed selection.<br /> Higher values tend to increase false negative and decrease false positives.<br />Configuration hint: corresponds to values of <em>smooth & scaled</em> image");
