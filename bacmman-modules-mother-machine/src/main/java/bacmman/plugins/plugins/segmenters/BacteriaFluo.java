...
public class BacteriaFluo extends BacteriaIntensitySegmenter<BacteriaFluo> {
    ...
    NumberParameter backgroundEdgeFusionThld = new BoundedNumberParameter("Background Edge Fusion Threshold", 4, 1.5, 0, null).setEmphasized(true).setHint("Threshold for background partition fusion. <br />Each pair of adjacent regions are merged if a criterion is inferior to this threshold.<br />Interface refers to the area of contact between two regions, Dec(E)@Inter refers to the first decile of edge (as defined by the <em>Edge Map</em> parameter) value at the interface, BCK_SD refers to the standard deviation of the background. Criterion is Dec(E)@Inter / BCK_SD<br />Sensible to sharpness of edges (decrease smoothing in edge map to increase sharpness)<br />Configuration Hint: Tune the value using intermediate image <em>Interface Values for Background Fusion</em>, no interface between foreground and background partitions should have a value under this threshold");
    ...
    public String getHintText() {
        return "<b>Intensity-based 2D segmentation of bacteria within microchannels:</b>"
            ...
            "<em>Region values after partitioning</em>: MIP after partitioning step. Importantly regions should be either located in foreground or in background but not overlap both areas<br />"
            +"<em>Interface Values for background partition fusion</em>: Each segment represent the area of contact between two partitions (referred to as interface) and its value is the criterion, to be compared with the parameter <em>Split Threshold</em>. Interface values should be as high as possible between foreground and background, and as low as possible between background regions<br />"
            +"<em>Region Values after background partition fusion</em>: MIP after merging background partition. No foreground regions should be merged with background</li>"
