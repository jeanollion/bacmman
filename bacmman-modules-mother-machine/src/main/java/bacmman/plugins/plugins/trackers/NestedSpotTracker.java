...
public class NestedSpotTracker implements TrackerSegmenter, TestableProcessingPlugin, Hint, HintSimple {
    ...
    static String calibrationTT = "<br />The distance parameter are calibrated: check the calibration of the images is correctly set, if not set it manually in the parameters of the position. See <a href='https://github.com/jeanollion/bacmman/wiki/Image-Import#Image-calibration'>Image calibration on bacmman's wiki</a>";
    ...
    ParentObjectClassParameter compartmentStructure = new ParentObjectClassParameter("Bacteria Object Class", -1, -1, false, false).setEmphasized(true).setHint("Indicate the name of the object class corresponding to bacteria, i.e. containing the spots to be tracked.");
    ...
    NumberParameter maxLinkingDistance = new BoundedNumberParameter("Maximum Linking Distance (FTF)", 2, 0.4, 0, null).setEmphasized(true).setHint("Maximum distance for frame-to-frame linking, (in µm). If two spots in two successive frames are separated by a distance larger than this distance, they cannot be linked together."+calibrationTT);
    NumberParameter maxLinkingDistanceGC = new BoundedNumberParameter("Maximum Linking Distance", 2, 0.75, 0, null).setEmphasized(true).setHint("Maximum distance for the gap-closing step, (in µm). If two spots in two different frames are separated by a distance larger than this distance, they cannot be linked together. An additional cost proportional to the gap is added to the distance between spots (see the <em>Gap Distance Penalty</em> parameter)"+calibrationTT);
    ...
    static String acknwoledge = "<br /><br />Linking is based on TrackMate's implementation (<a href='https://imagej.net/TrackMate'>https://imagej.net/TrackMate</a>) of u-track software (<a href='https://www.utsouthwestern.edu/labs/jaqaman/software/'>https://www.utsouthwestern.edu/labs/jaqaman/software/</a>)";

    @Override
    public String getSimpleHintText() {
        return toolTipSimple+ acknwoledge;
    }

    @Override
    public String getHintText() {
        return toolTipSimple + corrDesc+ acknwoledge;
    }
